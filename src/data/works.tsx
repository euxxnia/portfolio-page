...
UXUI는 어쨌든 어느정도 틀이 정해져있으니 늘 하던 것처럼 레퍼런스든 기억이든 떠올려봤고, 모바일 앱에서 특히 자주 볼 수 있는 '정보는 한번에 하나씩'식 디자인이랑 / 그냥 한꺼번에 뿌리는 방식 중에 선택해야 했다. 토스를 필두로 여러 앱들에서 사용자의 집중을 분산시키지 않게 적은 수의 질문으로 나누는 방식을 많이 사용하고 있고, 토스에서는 특히 '정보는 한번에 하나씩' 이걸 강조하고 있었다. 하지만 웹이라면 얘기가 달라지니 페이지는 정말 최소화하고 싶었고 정보가 선택사항? 그러면 한번에 주는 게 나으려나 이런 생각으로 초안을 만들던 중 팀원 중 한 명은 저 폼 형태 자체에 대해 반대했고, 소통 과정에서 원하는 바가 뭔지 알아내다가 논의가 조금 길어졌다.
...
그래서 다시 막막해졌다. 작년 말부터 **스톡 쓰는 거에 너무 거부감 느끼지 말자**는 생각을 하고 있었기 때문에 지피티를 포기하면서부터 바로 좋은 타로 이미지들을 찾아나섰다. 작업 기간이 2달(+a...) 이상이 되지 않는 한 내가 만들고 파인튜닝하는 건 말도 안 되는 거니..
...
이거 스타일 통일하는 건 수작업이었지만 당연히 할 의향이 있었고 배경색이랑 선색, 선 두께랑 불투명도 조절 이런 걸로 이미지 밀도를 어느정도 통일시키니 쓸 만해졌다.
